...
- Docs: Next FIELD Vertical Reuse + Gap Audit V1 — candidate on-site appliance/machine service = **CONFIG_ONLY** FIELD peer (proposed `appliance_service` / `field_service_appliance@v1` / `APPLIANCE_SERVICE` — **not registered**); FIELD bundle REUSABLE_WITH_CONDITIONS; Load/Soak/Chaos repeat **NO**; next wait `SERVICE_APPLIANCE_SERVICE_REFERENCE_VERTICAL_PROOF_V1`; no App Factory packaging yet; not GPS; 11/11 sales batch unchanged.
