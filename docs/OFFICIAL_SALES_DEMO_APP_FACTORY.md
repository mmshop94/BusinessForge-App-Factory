...
> **Latest public rebuild:** Push Device Registration V3-02 — **11/11** APK+AAB (historical). Slug list authority now **16** with `demo-bakery` + `demo-florist` + `demo-butcher` + `demo-farm-shop` + `demo-beverage-store`. Rebuild public batch after demo-plane bootstrap.
> **Status:** READY for bakery + florist + butcher + farm_shop + beverage_store slug discovery · APK rebuild deferred to demo ops
...
## Official Sales Demos (16)

Includes `demo-restaurant`, `demo-village-store`, `demo-bakery`, `demo-florist`, `demo-butcher`, `demo-farm-shop`, `demo-beverage-store`, and nine appointment sales demos (`demo-hairdresser` … `demo-pet-grooming`).
**Slug count:** **16** (`OFFICIAL_SALES_DEMO_SLUGS`). Technical `test-commerce-*` excluded.
