...
> **Latest public rebuild:** Push Device Registration V3-02 — **11/11** APK+AAB (historical). Slug list authority now **12** with `demo-bakery` (village_store consume). Rebuild public batch after demo-plane bootstrap includes bakery.
> **Status:** READY for bakery slug discovery · APK rebuild deferred to demo ops
...
Includes `demo-restaurant`, `demo-village-store`, `demo-bakery` (village_store@v1 + BAKERY), and nine appointment sales demos (`demo-hairdresser` … `demo-pet-grooming`).
**Slug count:** **12** (`OFFICIAL_SALES_DEMO_SLUGS`). Technical `test-commerce-bakery` excluded.
